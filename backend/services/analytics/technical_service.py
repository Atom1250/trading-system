...
from indicators.technicals import macd, rsi, sma
...
class TechnicalIndicatorsService:
    ...
    def calculate_indicators(
        ...
        sma_20_val = (
            float(df["sma_20"].iloc[-1]) if pd.notna(df["sma_20"].iloc[-1]) else None
        )
        sma_50_val = (
            float(df["sma_50"].iloc[-1]) if pd.notna(df["sma_50"].iloc[-1]) else None
        )
        sma_200_val = (
            float(df["sma_200"].iloc[-1]) if pd.notna(df["sma_200"].iloc[-1]) else None
        )

        if sma_20_val:
            signal = "buy" if current_price > sma_20_val else "sell"
            indicators.append({"name": "SMA_20", "value": sma_20_val, "signal": signal})

        if sma_50_val:
            signal = "buy" if current_price > sma_50_val else "sell"
            indicators.append({"name": "SMA_50", "value": sma_50_val, "signal": signal})

        if sma_200_val:
            signal = "buy" if current_price > sma_200_val else "sell"
            indicators.append(
                {"name": "SMA_200", "value": sma_200_val, "signal": signal},
            )

        # RSI
        rsi(df, period=14)
        rsi_val = (
            float(df["RSI_14"].iloc[-1]) if pd.notna(df["RSI_14"].iloc[-1]) else None
        )
        ...
        if rsi_val:
            ...
            indicators.append({"name": "RSI_14", "value": rsi_val, "signal": signal})

        # MACD
        macd(df, fast_span=12, slow_span=26, signal_span=9)
        macd_line = (
            float(df["MACD_line"].iloc[-1])
            if pd.notna(df["MACD_line"].iloc[-1])
            else None
        )
        macd_signal = (
            float(df["MACD_signal"].iloc[-1])
            if pd.notna(df["MACD_signal"].iloc[-1])
            else None
        )

        if macd_line and macd_signal:
            signal = "buy" if macd_line > macd_signal else "sell"
            indicators.append({"name": "MACD", "value": macd_line, "signal": signal})
