...
class BacktestService:
    ...
    def run_backtest(
        ...
        if df.empty:
            raise ValueError(f"No price data available for {symbol} in the specified date range")
        ...
        metrics = {
            "total_return": Decimal(str(stats.get("Return [%]", 0) / 100)) if stats.get("Return [%]") else Decimal("0"),
            "total_return_pct": Decimal(str(stats.get("Return [%]", 0))) if stats.get("Return [%]") else Decimal("0"),
            "sharpe_ratio": Decimal(str(stats.get("Sharpe Ratio", 0))) if stats.get("Sharpe Ratio") else None,
            "max_drawdown": Decimal(str(abs(stats.get("Max. Drawdown [%]", 0)) / 100)) if stats.get("Max. Drawdown [%]") else Decimal("0"),
            "max_drawdown_pct": Decimal(str(abs(stats.get("Max. Drawdown [%]", 0)))) if stats.get("Max. Drawdown [%]") else Decimal("0"),
            "win_rate": Decimal(str(stats.get("Win Rate [%]", 0))) if stats.get("Win Rate [%]") else None,
        ...
        if "equity_curve" in results:
            ec_df = results["equity_curve"]
            for date, value in ec_df.items():
                equity_curve.append({
