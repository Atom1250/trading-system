...
from db.models import PortfolioDB, PositionDB, TradeDB, PortfolioHistoryDB
from models.portfolio import (
    PortfolioCreate, Portfolio, Position, Trade,
    PortfolioMetrics, PortfolioHistory
)
...
def get_positions(db: Session, portfolio_id: int) -> List[Position]:
    ...
    positions = db.query(PositionDB).filter(PositionDB.portfolio_id == portfolio_id).all()
    return [Position.model_validate(p) for p in positions]


def get_trades(db: Session, portfolio_id: int, skip: int = 0, limit: int = 100) -> List[Trade]:
...
def calculate_metrics(db: Session, portfolio_id: int) -> PortfolioMetrics:
    ...
    total_pnl_pct = (total_pnl / portfolio.initial_capital) * 100 if portfolio.initial_capital > 0 else Decimal("0")
    ...
    if yesterday_history:
        ...
        daily_pnl_pct = (daily_pnl / yesterday_history.total_value) * 100 if yesterday_history.total_value > 0 else Decimal("0")
    else:
        daily_pnl = Decimal("0")
        daily_pnl_pct = Decimal("0")
    
    # Calculate max drawdown from history
    history = db.query(PortfolioHistoryDB).filter(PortfolioHistoryDB.portfolio_id == portfolio_id).all()
    if history:
    ...
    win_rate = (len(winning_trades) / num_trades * 100) if num_trades > 0 else Decimal("0")
...
def get_history(db: Session, portfolio_id: int, days: int = 30) -> List[PortfolioHistory]:
