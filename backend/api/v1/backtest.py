...
STRATEGIES = {
    ...
    "CandleCombined": CandleCombinedStrategy,
}
...
def run_backtest(request: BacktestRequest):
    ...
    try:
        ...
        if request.strategy_name not in STRATEGIES:
            raise HTTPException(
                status_code=400, detail=f"Strategy '{request.strategy_name}' not found."
            )
        ...
        start_dt = datetime.combine(request.start_date, datetime.min.time()).replace(
            tzinfo=timezone.utc
        )
        end_dt = datetime.combine(request.end_date, datetime.max.time()).replace(
            tzinfo=timezone.utc
        )
        ...
        metrics = BacktestMetrics(
            total_return=res_metrics.get("total_return", 0.0)
        ...
        equity_curve = []
        if (
            results.portfolio_history is not None
            and not results.portfolio_history.empty
        ):
        ...
        # 7. Generate Execution Chart
        chart_b64 = None
        try:
            plotter = PerformancePlotter()
            fig = plotter.plot_trade_execution(
                data=results.data,
            ...
            print(f"Warning: Failed to generate execution chart: {e}")
