...
async def export_to_google_sheets(
    portfolio_id: int,
    spreadsheet_name: Optional[str] = Query(None, description="Name of the spreadsheet"),
    ...
    try:
            ...
            "positions": [
                {
                    ...
                    "market_value": float(p.current_price * p.quantity) if p.current_price else 0,
        ...
        result = google_sheets_service.export_portfolio(portfolio_data, spreadsheet_name)
        return result
...
async def export_file(
    ...
    try:
        ...
        if format == "excel": filename = f"portfolio_{portfolio_id}.xlsx"
