    ...
    def __post_init__(self):
        ...
        if self.param_type == "categorical":
            if not self.categorical_values:
                raise ValueError(f"categorical_values required for categorical parameter {self.name}")
        else:
            if self.min_value is None or self.max_value is None:
                raise ValueError(f"min_value and max_value required for {self.param_type} parameter {self.name}")
