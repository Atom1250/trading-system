...
from strategy_lab.strategies.base import Strategy, Signal, SignalType
from strategy_lab.config import StrategyConfig
from strategy_lab.risk.engine import RiskEngine, RiskViolation
from strategy_lab.risk.portfolio_state import PortfolioState, PositionState, PositionSide
...
from strategy_lab.factors.base import FactorRegistry, Factor
...
class StrategyBacktestEngine:
    ...
    def __init__(
        ...
        self.factor_registry = factor_registry # Using global registry mostly, but kept for DI
    ...
    def run(
        ...
        **kwargs
        ...
        for symbol in universe:
            market_data[symbol] = self.data_provider.get_history(symbol, start_date, end_date)
            
        if not market_data:
            raise ValueError("No data found for universe")

        # Combine timeline (union of all timestamps)
        all_timestamps = sorted(list(set().union(*[d.df.index for d in market_data.values()])))
        
        # 2. Compute Factors (Pre-computation)
        print("Pre-computing factors...")
        factor_panels: Dict[str, pd.DataFrame] = self._compute_factors(strategy, market_data)
        ...
        for timestamp in all_timestamps:
            ...
            current_ret = (portfolio.current_equity - prev_equity) / prev_equity if prev_equity > 0 else 0
            ...
            for symbol, signal_val in signals_at_time.items():
                ...
                # Get current price
                if symbol not in market_data or timestamp not in market_data[symbol].df.index:
                    continue
                ...
                is_close = False # Logic to determine if closing
                ...
                if portfolio.has_position(symbol):
                    ...
                    if (pos.side == PositionSide.LONG and signal_val < 0) or \
                       (pos.side == PositionSide.SHORT and signal_val > 0):
                        # Close existing
                        self._execute_close(portfolio, symbol, price, timestamp, trade_log)
                        is_close = True
                ...
                try:
                    ...
                    if not portfolio.has_position(symbol):
                        ...
                        # D. Execute Trade
                        self._execute_open(portfolio, proposed_pos, price, timestamp, trade_log)
    ...
    def _compute_factors(self, strategy: Strategy, market_data: Dict[str, MarketDataSlice]) -> Dict[str, pd.DataFrame]:
    ...
    def _get_signals_at_timestamp(self, raw_signals: Dict[str, pd.Series], timestamp) -> Dict[str, float]:
    ...
    def _update_portfolio_valuation(
        ...
        for symbol, pos in portfolio.open_positions.items():
            if symbol in market_data and timestamp in market_data[symbol].df.index:
                current_price = Decimal(str(market_data[symbol].df.loc[timestamp, 'close']))
    ...
    def _execute_open(
        ...
        trade_log.append({
            ...
            'type': 'BUY_OPEN' if position.side == PositionSide.LONG else 'SELL_OPEN',
            'price': price,
    ...
    def _execute_close(
        ...
        if pos.side == PositionSide.LONG:
            ...
            returned_capital = (pos.quantity * (2 * pos.avg_price - exit_price)) # Crude approx for short margin return
