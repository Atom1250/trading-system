...
class BacktestResults:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        # Profit factor
        if self.trade_log is not None and not self.trade_log.empty and 'pnl' in self.trade_log.columns:
        ...
        for timestamp, signal in trades.items():
            if timestamp in self.data.index:
                ...
                log.append({
                    ...
                    'signal': signal,
                    'price': price,
                    'type': 'LONG' if signal > 0 else 'SHORT' if signal < 0 else 'CLOSE'
                })
  ...
  Expectancy:         {metrics['expectancy']:.4f}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'trade_log': self.get_trade_log_df().to_dict('records') if not self.get_trade_log_df().empty else [],
            'config': {
