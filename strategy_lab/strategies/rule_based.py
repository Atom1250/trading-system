...
from .base import Strategy, MarketDataSlices, FactorPanels, SignalType
...
class MultiSignalRuleStrategy(Strategy):
    ...
    def generate_signals(
        ...
        for symbol, factors_df in factor_panels.items():
            ...
            tech_score = factors_df['technical'] if 'technical' in factors_df.columns else pd.Series(0.0, index=factors_df.index)
            fund_score = factors_df['fundamental'] if 'fundamental' in factors_df.columns else pd.Series(0.0, index=factors_df.index)
            sent_score = factors_df['sentiment'] if 'sentiment' in factors_df.columns else pd.Series(0.0, index=factors_df.index)
