...
class VolumeMoveBreakoutStrategy(Strategy):
    ...
    def generate_signals(
        ...
        avg_vol_window = params.get("avg_volume_window", 20) # Used if computing locally, but we might rely on factors
            ...
            for i in range(avg_vol_window, len(df)):
                ...
                if position == 0:
                    ...
                    if pct_change >= tp_pct:
                        ...
                        position = 0
                        entry_price = 0.0
                    elif pct_change <= sl_pct: # sl_pct is negative, e.g. -0.10
                        output_signal = -1 # Close Long
                        position = 0
                        entry_price = 0.0
