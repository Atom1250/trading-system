...
from strategy_lab.config import RiskConfig, RiskConstraintConfig, CapitalAllocationMode, EntryMode
...
class RiskEngine:
    ...
    def _check_entry_mode(self, symbol: str, side: PositionSide, portfolio: PortfolioState) -> None:
        ...
        if portfolio.has_position(symbol):
            ...
            if current_pos.side == side:
                if self.risk_config.entry_mode != EntryMode.PYRAMID:
                     raise RiskViolation(f"Pyramiding not allowed (Mode: {self.risk_config.entry_mode})")
    ...
    def _check_max_drawdown(self, portfolio: PortfolioState) -> None:
        ...
        """
        if float(portfolio.current_drawdown_pct) / 100.0 > self.risk_config.max_drawdown_pct:
            raise RiskViolation(
