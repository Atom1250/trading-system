...
from strategy_lab.config import StrategyConfig, RiskConfig
...
def optimize_lab_strategy(
    ...
    n_trials: int,
    param_ranges: Dict[str, Tuple[Any, Any, str]], # name -> (min, max, type='int'/'float')
    ...
    def objective(trial):
        ...
        risk_cfg = RiskConfig(
            ...
            risk_per_trade=0.01,
            stop_loss_atr_multiple=full_params.get('stop_loss_atr_multiple', 2.0) # If not in params, use default
        )
        ...
        from strategy_lab.risk.engine import RiskEngine # Local import to avoid circular if any
