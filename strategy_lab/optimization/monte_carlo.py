...
from strategy_lab.config import OptimizationConfig, StrategyConfig, RiskConfig, RiskConstraintConfig
...
class MonteCarloOptimizer:
    ...
    def optimize(
        ...
        print(f"Starting Monte Carlo optimization with {self.config.n_trials} trials...")
        ...
        for i in range(self.config.n_trials):
            ...
            try:
                ...
                is_valid = self._check_constraints(metrics, base_strategy_config.risk_constraints)
                # Also check risk limits directly if they are in metrics
                if metrics.get('max_drawdown', 0) > base_strategy_config.risk_config.max_drawdown_pct:
                     is_valid = False
                ...
                print(f"Trial {i+1}/{self.config.n_trials}: Valid={is_valid}, Return={metrics['total_return']:.2%}")
    ...
    def _check_constraints(self, metrics: Dict[str, Any], constraints: RiskConstraintConfig) -> bool:
