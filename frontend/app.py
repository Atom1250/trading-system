...
with st.sidebar.expander("💼 Portfolio Management", expanded=False):
    ...
    new_pf_capital = st.number_input("Initial Capital", value=100000.0, step=1000.0, key="new_pf_capital")
...
# Main content
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["📈 Chart", "🎯 Strategies", "📡 Signals", "🔬 Backtests", "⚙️ Optimization", "🤖 AI Insights", "🔌 Integration"])
    ...
    try:
        ...
        if price_data and price_data.get("data"):
            ...
            if show_sma:
                ...
                fig.add_trace(go.Scatter(x=df_prices['date'], y=df_prices['SMA_20'], name='SMA 20', line=dict(color='orange', width=1)))
                fig.add_trace(go.Scatter(x=df_prices['date'], y=df_prices['SMA_50'], name='SMA 50', line=dict(color='blue', width=1)))

            if show_ema:
                df_prices['EMA_20'] = df_prices['close'].ewm(span=20, adjust=False).mean()
                fig.add_trace(go.Scatter(x=df_prices['date'], y=df_prices['EMA_20'], name='EMA 20', line=dict(color='purple', width=1)))

            if show_bb:
                window = 20
                std_dev = 2
                df_prices['BB_Middle'] = df_prices['close'].rolling(window=window).mean()
                df_prices['BB_Std'] = df_prices['close'].rolling(window=window).std()
                df_prices['BB_Upper'] = df_prices['BB_Middle'] + (df_prices['BB_Std'] * std_dev)
                df_prices['BB_Lower'] = df_prices['BB_Middle'] - (df_prices['BB_Std'] * std_dev)
                
                fig.add_trace(go.Scatter(x=df_prices['date'], y=df_prices['BB_Upper'], name='BB Upper', line=dict(color='gray', width=1, dash='dash')))
                fig.add_trace(go.Scatter(x=df_prices['date'], y=df_prices['BB_Lower'], name='BB Lower', line=dict(color='gray', width=1, dash='dash'), fill='tonexty', fillcolor='rgba(128, 128, 128, 0.1)'))
                fig.add_trace(go.Scatter(x=df_prices['date'], y=df_prices['BB_Middle'], name='BB Middle', line=dict(color='gray', width=1, dash='dot')))
...
with tab3:
    ...
    try:
        if signal_type == "Aggregated":
            ...
            st.metric("Combined Score", f"{score}/100", f"Recommendation: {rec.upper()}")
                ...
                if signals.get("technical"):
                    ...
                    if tech.get("indicators"):
                        ...
                        for ind in tech["indicators"]:
                            st.write(f"- {ind['name']}: {ind['value']:.2f} ({ind.get('signal', 'N/A')})")
            ...
            st.metric("Overall Sentiment", f"{float(signals.get('overall_sentiment', 0)):.2f}")
...
with tab4:
    ...
    with col1:
        try:
            ...
            selected_strategy = st.text_input("Strategy Name", value="moving_average_crossover")
            custom_params = {}
    ...
    with col2:
        ...
        commission = st.number_input("Commission (%)", min_value=0.0, value=0.1, step=0.01) / 100
    ...
    if st.button("Run Backtest", type="primary"):
        try:
            ...
            with col1:
                st.metric("Total Return", f"{float(metrics.get('total_return_pct', 0)):.2f}%")
            with col2:
                sharpe = metrics.get('sharpe_ratio')
                st.metric("Sharpe Ratio", f"{float(sharpe):.2f}" if sharpe else "N/A")
            with col3:
                st.metric("Max Drawdown", f"{float(metrics.get('max_drawdown_pct', 0)):.2f}%")
            with col4:
                ...
                if trades:
                    ...
                    if 'entry_date' in df_trades.columns:
                        ...
                        fig.add_trace(go.Scatter(
                            ...
                            marker=dict(
                                size=10,
                                color=['green' if pnl > 0 else 'red' for pnl in df_trades.get('pnl', [0]*len(df_trades))],
                                symbol='triangle-up'
                            )
                        ))
...
with tab5:
    ...
    with col1:
        try:
            strategies = api.get_strategies()
            strategy_names = [s["name"] for s in strategies]
            opt_strategy = st.selectbox("Strategy to Optimize", options=strategy_names, key="opt_strategy")
            
            # Dynamic parameter ranges based on strategy
            param_ranges = {}
            if opt_strategy:
                strategy_info = next((s for s in strategies if s["name"] == opt_strategy), None)
                if strategy_info:
                    ...
                    for param_name, param_value in default_params.items():
                        if isinstance(param_value, (int, float)):
                            with st.expander(f"{param_name}", expanded=True):
                                p_type = "int" if isinstance(param_value, int) else "float"
                                
                                c1, c2, c3 = st.columns(3)
                                with c1:
                                    min_val = st.number_input(f"Min", value=float(param_value)*0.5, key=f"min_{param_name}")
                                with c2:
                                    max_val = st.number_input(f"Max", value=float(param_value)*1.5, key=f"max_{param_name}")
                                with c3:
                                    step_val = st.number_input(f"Step", value=1.0 if p_type=="int" else 0.1, key=f"step_{param_name}")
    ...
    with col2:
        ...
        n_trials = st.number_input("Number of Trials", min_value=5, max_value=100, value=20, step=5)
        opt_metric = st.selectbox("Metric to Optimize", options=["sharpe_ratio", "total_return_pct", "max_drawdown_pct"])
        ...
        if st.button("Run Optimization", type="primary"):
            if not param_ranges:
                ...
                try:
                    ...
                    st.success(f"Optimization complete! Best Value: {result['best_value']:.4f}")
                    ...
                    if result.get("trials"):
                        ...
                        df_trials = pd.concat([df_trials.drop('params', axis=1), df_params], axis=1)
                        
                        st.dataframe(df_trials.sort_values('value', ascending=(opt_direction=="minimize")), use_container_width=True)
                        
                        # Plot optimization history
                        fig = px.scatter(df_trials, x='number', y='value', title="Optimization History")
                        st.plotly_chart(fig, use_container_width=True)
    ...
    try:
        ...
        if portfolios:
            ...
            with col1:
                ...
                sheet_name = st.text_input("Spreadsheet Name", value=f"Portfolio Export - {selected_portfolio_name}")
                
                if st.button("Export to Google Sheets"):
                    try:
                        with st.spinner("Exporting..."):
                            result = api.export_to_google_sheets(selected_portfolio_id, sheet_name)
                        st.success(f"Export successful! [Open Sheet]({result['spreadsheet_url']})")
                    except Exception as e:
                ...
                if st.button("Prepare Download"):
                    try:
                        with st.spinner("Generating file..."):
                            file_content = api.download_portfolio_export(selected_portfolio_id, format_type.lower())
