...
def train_performance_model(csv_path: str):
    ...
    print(
        f"Trained RandomForestRegressor on {len(X_train)} samples; R^2 on hold-out: {r2:.4f}",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Offline ML analysis for Optuna strategy results",
    )
    parser.add_argument(
        "--csv",
        required=True,
        help="Path to Optuna results CSV (e.g., reports/optuna_ma_AAPL.csv)",
    )
    parser.add_argument(
        "--clusters",
        type=int,
